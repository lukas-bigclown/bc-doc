...
+
 plugin/led-strip/data/set" {"color": [255,0,0,0]}

* Use data/set color attribute to generate pixels:
+
 plugin/led-strip/data/set" {"state": "color"}

* Use rules matching to generate pixels:
+
 plugin/led-strip/data/set" {"state": "rules"}

==== Examples
* Use LED strip as light
+
 mosquitto_pub -t "plugin/led-strip/config" -m '{"state": "color", "color": [0, 0, 0, 255]}], "brightness": 200}'
