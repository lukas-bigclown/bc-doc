...
+ 
 mosquitto_pub -t "plugins/led-strip/data/set" -m '{"state": "color", "color": [0, 0, 0, 128]}'

. Update and retain LED strip brightness:
+
 mosquitto_pub -t "plugins/led-strip/data/set" '{"brightness": 100}' -r
...
+ 
 mosquitto_pub -t "plugins/led-strip/data/set" -m '{"state": "rules"}'
...
+
 plugins/led-strip/data/set" {"color": [255,0,0,0]}

* Use data/set color attribute to generate pixels:
+
 plugins/led-strip/data/set" {"state": "color"}

* Use rules matching to generate pixels:
+
 plugins/led-strip/data/set" {"state": "rules"}

==== Examples
* Use LED strip as light
+
 mosquitto_pub -t "plugins/led-strip/config" -m '{"state": "color", "color": [0, 0, 0, 255]}], "brightness": 200}'

* Change LED strip brightness:
+
 mosquitto_pub -t "plugins/led-strip/data/set" '{"brightness": 100}'

* Switch LED strip OFF
+
 mosquitto_pub -t "nodes/base/led-strip/-/set" -m '{"state": false}' 

* Switch LED strip ON
+
 mosquitto_pub -t "nodes/base/led-strip/-/set" -m '{"state": true}' 

NOTE: Pixels are preserved.

* Request LED strip state
+
 mosquitto_sub -v -t "nodes/base/led-strip/-" &
 mosquitto_pub -t "nodes/base/led-strip/-/get" -m '{}' 
 nodes/base/led-strip/- {"state": true}
...
+
 mosquitto_pub -t "plugins/led-strip/config" -m '{
  "values": [
...
+
 mosquitto_pub -t "plugins/blynk/config" -m '{"token":"ec9fdfdf0c7d49bcae3e83be3dceb4c1"}' -r
